...
- **Toán tử ba ngôi (`? :`)**: Hạn chế sử dụng nếu logic phức tạp. Thay vào đó, hãy sử dụng cấu trúc `if-else` rõ ràng.
  ...
  - *Khuyến khích:* Dùng if-else rõ ràng.
...
- Viết hoa chữ cái đầu cho Models, Methods (VD: `AddShowtimeAsync`).
- Viết thường chữ cái đầu (camelCase) cho các biến cục bộ (VD: `movieName`, `request`).
...
- Các hàm Service **bắt buộc** trả về cấu trúc Tuple `(bool IsSuccess, string Message, object? Data)` để Controller dễ dàng bóc tách và phản hồi cho Frontend.
- Ví dụ: `return (true, "Thêm thành công", newObj.Id);`